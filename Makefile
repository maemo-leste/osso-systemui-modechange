...
libsystemuiplugin_modechange.so: osso-systemui-modechange.c
	$(CC) $^ -o $@ -shared $(CFLAGS) $(LDFLAGS) $(shell pkg-config --libs --cflags osso-systemui hildon-1 gconf-2.0 gtk+-2.0 dbus-1 glib-2.0) -Wl,-soname -Wl,$@ -Wl,-rpath -Wl,/usr/lib/hildon-desktop
